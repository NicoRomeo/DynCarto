...
function main(n_sys::Int, num_IC_warmup::Int,num_IC::Int, D::Float64, t_max::Float64, N_times::Int, fname::String,)
    ...
    for n = ProgressBar(1:n_sys, printing_delay=1.0)
        ...
        a, b, X, Y, tau = sample_system()
        fun!, g!, jac! = make_sys_SDE(a,b, D, X, Y, tau)
        times = LinRange(0,t_max, N_times)
        f! = ODEFunction(fun!, jac = jac!)
        trajectories_warmup = Array{Float64, 3}(undef, num_IC_warmup, length(times), 2)
        ...
        # first integrate a few trajectories starting from fixed points
        for i = 1:num_IC_warmup
            if length(fixed_points) > 0
                n_fp = rand(1:length(fixed_points))
                u0 = fixed_points[n_fp]
            else
            u0 = zeros(Float64, 2)
            end
            ...
            trajectories_warmup[i,:,1] = X*getindex.(sol.u, 1)
            trajectories_warmup[i,:,2] = Y*getindex.(sol.u, 2)
        end
        # then use the mean, std positions to sample initial conditions 
        mean_ic_x = mean(trajectories_warmup[:,:,1])
        mean_ic_y = mean(trajectories_warmup[:,:,2])
        
        std_idx_x = std(trajectories_warmup[:,:,1])
        std_idx_y = std(trajectories_warmup[:,:,2])
        
        for i = 1:num_IC
            u0 = [2*std_idx_x*randn()+mean_ic_x,2*std_idx_y*randn()+mean_ic_y]
...
rng = MersenneTwister(123468);


n_sys = 50000
num_IC_warmup = 10 # 1200 # 30 # 1200 # 30 
num_IC = 30 # 1200 # 30 # 1200 # 30 
D = 0.1 #diffusion constant
t_max = 50. # 0.1 #50 # 0.1 
N_times = 400 # 10 # 400 # 10 
fname = "dataSDE$(n_sys)_ICtest_tmax50_n30_D0d1.h5"

main(n_sys, num_IC_warmup, num_IC, D, t_max, N_times, fname)
