...
function main(n_sys::Int, num_IC::Int, D::Float64, t_max::Float64, N_times::Int, fname::String, Kmin::Float64, Kmax::Float64, Am_min::Float64, Am_max::Float64, alpha_p::Float64)

    dico = Dict{Int, Vector{Vector{Float64}}}()
    K_range = LinRange(Kmin, Kmax, n_sys)
    Am_range = LinRange(Am_min, Am_max, n_sys)
    Ks = [k for k in K_range, am in Am_range ]
    Ams = [am for k in K_range, am in Am_range ]
    alpha_p = 1.0
    fid = h5open(fname, "w")
...
fname = "SNIChopfSDE1024_tmax50_n30_D0d005.h5"

main(n_sys, num_IC, D, t_max, N_times, fname, Kmin, Kmax, Am_min, Am_max, alpha_p)
